...
def cyton_function(port = None, cmdque = None, dataque = None):
    cton = Cyton(port,cmdque,dataque)
    cton.mainloop()
...
class Cyton(object):
    ...
    def __init__(self,port = None, cmdque = None, dataque = None):
        ...
        self.virgin = True
        self.streaming_ok_ev = Event()
        self.streaming_ok_ev.clear()

    def start_record(self):
       self.stream_thread.start()
       self.stpEv2.clear()
       self.streaming_ok_ev.wait()
    ...
    def callback(self,sample):
        ...
        self.tem_timestamp_buffer.append(self.c_stamp)
        if len(self.tem_data_buffer) == 25: #约100ms
            self.dataque.put([self.tem_data_buffer,self.tem_timestamp_buffer[0]])      #多进程que发送数据和头部对应的时间戳
            self.tem_data_buffer = []
            self.tem_timestamp_buffer = []
    ...
    def start_streaming(self, lapse=-1):
        ...
        while self.streaming:
            if self.virgin:
                self.streaming_ok_ev.set()  #告知启动完成
                self.virgin = False

            # read current sample
            sample = self._read_serial_binary()
            self.c_stamp = timestamp.getstamp()
